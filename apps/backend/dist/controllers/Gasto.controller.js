...
export const obtenerGastos = async (req, res) => {
    try {
        ...
        const { rows } = await pool.query('SELECT id, usuario_id, descripcion, monto, fecha, categoria FROM gastos WHERE usuario_id = $1 ORDER BY fecha DESC, id DESC', [usuarioId]);
        res.json(rows);
    }
    catch (error) {
...
export const eliminarGasto = async (req, res) => {
    try {
        const usuarioId = req.user.userId;
        const { id } = req.params;
        const result = await pool.query('DELETE FROM gastos WHERE id = $1 AND usuario_id = $2', [id, usuarioId]);
